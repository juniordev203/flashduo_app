  ...
  <div class="h-full w-full flex flex-col bg-slate-50">
    ...
    <div id="chat-container" class="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      >
        ...
        <h2 class="text-lg font-medium">Chào mừng đến với FlashDuo AI</h2>
      ...
      >
        <div
          class="max-w-3/4 rounded-lg shadow-sm p-3"
          :class="{
      ...
      <div v-if="store.isLoading" class="flex">
        <div class="max-w-3/4 bg-white rounded-lg shadow-sm p-4">
          <div class="font-medium mb-2">FlashDuo AI đang nhập...</div>
    ...
    <div class="border-t border-gray-300 bg-white p-4">
      <form @submit.prevent="store.sendMessage" class="flex space-x-2">
        ...
        >
          <span v-if="!store.isLoading">Gửi</span>
          <svg
            v-else
      ...
      <div class="text-xs text-gray-500 mt-1 text-center">
        @2025 FlashDuo AI by Junior@203
      </div>
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  if (store.messages.length === 0) {
    store.addMessage(
      "assistant",
      "Xin chào! Tôi là FlashDuo AI. Tôi có thể giúp gì cho bạn hôm nay?"
    );
  }
});
</script>
