  ...
  <div class="h-full w-full flex flex-col bg-gray-100">
    ...
    <div class="h-full w-full overflow-hidden p-4 flex flex-col gap-5">
      ...
      <ExamHeader :timeRemaining="examStore.timeRemaining" :examInfo="examStore.currentExam"
        @toggle-section="toggleSection" class="bg-white p-3 rounded-xl shadow-sm" />
      ...
      <div class="flex flex-col gap-4 h-full">
        ...
        <div class="flex-1 flex flex-col justify-between bg-white shadow-sm rounded-xl overflow-hidden">
          ...
          <div class="px-3 py-4 flex-1 border-blue-400 border-2 rounded-lg m-1">
            <!-- listening -->
            <ListeningSection v-if="examStore.currentSection === QuestionSectionEnum.Listening"
              :questions="examStore.listeningQuestion" :section="1" :currentIndex="getListeningIndex()"
              :answers="examStore.answers" @save-answer="saveAnswer" @change-question="setCurrentQuestion" />
            <!-- reading -->
            <ReadingSection v-if="examStore.currentSection === QuestionSectionEnum.Reading"
              :questions="examStore.readingQuestion" :section="2" :currentIndex="getReadingIndex()"
              :answers="examStore.answers" @save-answer="saveAnswer" @change-question="setCurrentQuestion" />
          </div>
        ...
        <div class="bg-white shadow-sm rounded-xl">
          <div class="p-4 flex flex-col gap-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button v-for="index in totalQuestionListening" :key="index - 1" @click="setCurrentQuestion(index - 1)"
              ...
              <button v-for="index in totalQuestionReading" :key="index - 1"
