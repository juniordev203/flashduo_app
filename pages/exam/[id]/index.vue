    ...
    <div class="h-full w-full flex flex-col bg-slate-50">
        ...
        <div class="h-full w-full overflow-auto p-4 flex flex-col gap-6 items-center justify-center">
            <div v-if="examDetail"
                ...
                <h1 class="text-2xl font-bold text-gray-800 mb-4">{{ examDetail.examName }}</h1>
                <div class="flex flex-col gap-2 items-center">
                    <p><strong>Thời gian làm bài:</strong> <span class="text-red-600">120 phút</span></p>
                    <p><strong>Số câu hỏi:</strong> <span class="text-red-600">{{ examDetail.totalQuestions }}</span>
                    </p>
                    <p><strong>Loại đề:</strong> TOEIC 2 kỹ năng</p>
                    <p><strong>Mô tả:</strong> Đề thi TOEIC gồm 2 phần chính: Listening và Reading. Bạn cần hoàn thành
                        tất
                        cả các
                        câu hỏi trong thời gian quy định.</p>
                </div>
                <NuxtLink v-if="examDetail && examDetail.id" class="w-full" :to="`/exam/${examDetail.id}/do`">
                    <button class="w-full py-2 text-lg font-semibold text-white bg-indigo-500 rounded transition-transform duration-150 active:scale-95 touch-manipulation">
                        Làm bài
                    </button>
                </NuxtLink>
            </div>
...
const featchExamDetail = async () => {
    try {
        const response = await examApiUtil.apiExamExamDetailExamIdGet(Number(examId));
