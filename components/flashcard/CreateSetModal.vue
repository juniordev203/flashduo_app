<template>
  <el-dialog :model-value="visible" @update:model-value="$emit('update:visible', $event)"
    :title="$t('lang_core_flashcard_create_set')" width="90%" :before-close="handleClose" class="rounded-lg">
    <div class="flex flex-col gap-4">
      <!-- Set Name -->
      <div class="space-y-2">
        <label class="text-sm font-medium text-gray-700">Tên bộ thẻ</label>
        <el-input v-model="formData.setName" placeholder="Nhập tên bộ thẻ..." :maxlength="50" show-word-limit
          class="w-full">
          <template #prefix>
      ...
      <div class="space-y-2">
        ...
        <el-input v-model="formData.description" type="textarea" :rows="3" placeholder="Nhập mô tả cho bộ thẻ..."
      ...
      <div class="space-y-2">
        ...
        <el-select v-model="formData.folderId" placeholder="Chọn thư mục" class="w-full">
          <el-option v-for="folder in folders" :key="folder.id" :label="folder.folderName" :value="folder.id || ''" />
        </el-select>
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <el-switch v-model="formData.isPublic" active-text="Công khai" inactive-text="Riêng tư" />
      </div>
    </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    showToast('success', t('lang_core_messages.success_create_set'));
    emit('created');
    handleClose();
  } catch (err) {
    console.error('Lỗi khi tạo bộ thẻ:', err);
    showToast('error', t('lang_core_messages.error_create_set'));
  }
};
</script>
